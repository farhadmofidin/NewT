...
body.push(spacer(80));
body.push(p("این پروژه در چهار فاز و مجموعاً ۵۵ روز اجرا می‌شود و پس از تحویل، تا یک ماه پشتیبانی رایگان ارائه می‌گردد.", { bold: true, color: NAVY }));
body.push(spacer(40));
body.push(dataTable(
  ["عنوان", "شرح"],
  [
    ["مدت کل اجرا", "۵۵ روز (رمزگشایی ۱۰، انبار داده ۱۰، مایگریشن ۲۰، BI ۱۵ روز)"],
...
body.push(dataTable(
  ...
  [
    ["پلتفرم عملیات", "ثبت دستور کار سرویس، مشتریان/قراردادها، صورت‌حساب و گواهینامه", "دستور کار (LR/LB/LS/FF/ND/UT/CI-LT/SE/PR)، اقلام مصرفی، صورت‌حساب، شعبه"],
...
body.push(spacer(60));
body.push(p("این تنها یک نمونه است. وقتی سه سامانه جدا هستند، مدیریت به ساده‌ترین پرسش‌ها هم پاسخ قطعی ندارد — هر پرسش، پاسخش در یک سامانهٔ دیگر است:"));
body.push(spacer(40));
body.push(dataTable(
  ["پرسشِ بی‌پاسخِ مدیریت", "چرا امروز بی‌پاسخ است"],
  [
    ["سود واقعیِ هر دستور کار چقدر است؟", "درآمد در صورت‌حساب، هزینه در تنخواه و انبار پخش است؛ کنار هم نمی‌آید."],
    ["موجودی واقعیِ انبار چقدر است و چقدرش کسری/پرت دارد؟", "مصرفِ عملیات با کسرِ انبار تطبیق داده نمی‌شود."],
    ["کدام شعبه سودده است و کدام زیان‌ده؟", "عملکرد شعب روی یک معیارِ مشترک مقایسه نمی‌شود."],
    ["آیا همهٔ صورت‌حساب‌های صادره ثبت و وصول شده‌اند؟", "صورت‌حساب در عملیات و سند در حسابداری است؛ جامانده‌ها پیدا نمی‌شوند."],
    ["هزینهٔ هر تنخواه به کدام دستور کار خورده است؟", "تخصیصِ هزینه به عملیات مبهم است و بهای تمام‌شده اشتباه می‌شود."],
    ["چند قلمِ انقضادار/سریال‌دار داریم و کجا مصرف شده؟", "ردیابیِ اقلام حیاتی بین عملیات و انبار وجود ندارد."],
    ["مطالبات معوق از مشتریان چقدر است؟", "وضعیت وصول و کسورات بین صورت‌حساب و حسابداری یکپارچه نیست."],
    ["چرا عددِ گزارش عملیات با گزارش مالی فرق دارد؟", "هر سامانه یک «حقیقتِ» متفاوت گزارش می‌کند."],
  ],
  [3900, 5460],
  { align: [AlignmentType.RIGHT, AlignmentType.RIGHT], boldCols: [0] }
));
body.push(spacer(60));
body.push(p("این پروپوزال همهٔ این پرسش‌ها را به پاسخ‌های عددی، خودکار و لحظه‌ای تبدیل می‌کند.", { bold: true, color: NAVY }));
...
// Phase 1 — Database decryption
body.push(h2("فاز اول — رمزگشایی دیتابیس سه نرم‌افزار | ۱۰ روز"));
body.push(p("پیش از هر کاری، ساختار پنهانِ دیتابیس هر سه نرم‌افزار باید باز و مستند شود؛ بدون این نقشه، استخراج داده ممکن نیست."));
body.push(bullet("مهندسی معکوس و رمزگشایی ساختار دیتابیس هر سه نرم‌افزار (جداول، روابط، فیلدها)."));
body.push(bullet("شناسایی محل ذخیرهٔ دستور کار، اقلام، صورت‌حساب، اسناد مالی و موجودی."));
body.push(bullet("باز کردن داده‌های رمزنگاری‌شده/کدشده و برقراری دسترسی امن به آن‌ها."));
body.push(bullet("تهیهٔ دیکشنری داده و نقشهٔ کاملِ سه دیتابیس."));
body.push(runs([t("خروجی فاز: ", { bold: true, color: TEAL }), t("نقشه و دیکشنری کاملِ دیتابیس سه نرم‌افزار — پیش‌نیازِ استخراج داده.")], { after: 60 }));

body.push(spacer(80));
// Phase 2 — Data Warehouse
body.push(h2("فاز دوم — ساخت انبار داده (Data Warehouse) | ۱۰ روز"));
...
body.push(spacer(80));
// Phase 2
body.push(h2("فاز سوم — مایگریشن و متناظرسازی (Migration & Mapping) | ۲۰ روز"));
...
body.push(spacer(80));
// Phase 3
body.push(h2("فاز چهارم — پلتفرم BI و داشبوردها | ۱۵ روز"));
...
body.push(dataTable(
  ["فاز", "تحویل‌شدنی اصلی"],
  [
    ["فاز ۱", "نقشه و دیکشنری کاملِ دیتابیس سه نرم‌افزار"],
    ["فاز ۲", "انبار دادهٔ مرکزی یکپارچه + مستندات مدل داده و فرآیند ETL"],
    ["فاز ۳", "موتور و گزارش مغایرت‌گیری سه‌جانبهٔ خودکار + دادهٔ تاریخی متناظرشده"],
    ["فاز ۴", "پلتفرم BI با داشبوردهای عملیاتی، انباری، مالی و مغایرت‌گیری + آموزش کاربران"],
...
body.push(dataTable(
  ["ردیف", "شرح", "مبلغ (تومان)"],
  [
    ["۱", "اجرای فاز اول — رمزگشایی دیتابیس سه نرم‌افزار (۱۰ روز)", "۵۵٬۰۰۰٬۰۰۰"],
    ["۲", "اجرای فاز دوم — ساخت انبار داده (۱۰ روز)", "۸۴٬۵۰۰٬۰۰۰"],
    ["۳", "اجرای فاز سوم — مایگریشن و متناظرسازی (۲۰ روز)", "۱۴۳٬۰۰۰٬۰۰۰"],
    ["۴", "اجرای فاز چهارم — پلتفرم BI و داشبوردها (۱۵ روز)", "۱۱۷٬۰۰۰٬۰۰۰"],
    ["", "جمع کل قرارداد اجرا (چهار فاز)", "۳۹۹٬۵۰۰٬۰۰۰"],
...
body.push(spacer(60));
body.push(p("پرداخت مبلغ هر فاز متناسب با تحویل همان فاز انجام می‌شود؛ مبلغ کل قرارداد اجرا ۳۹۹٫۵ میلیون تومان است.", { size: 20, color: GREY }));
body.push(spacer(60));
...
body.push(p("این پروپوزال یک مسیر روشن و مرحله‌ای برای عبور از وضعیت «سه سامانهٔ ناهماهنگ» به وضعیت «یک تصویر واحد، شفاف و قابل‌کاوش» ارائه می‌کند. در پایان ۵۵ روز، مدیریت شرکت صنعتی و خدمات مهندسی ایران خواهد توانست هم‌خوانی انبار، عملیات و حسابداری را به‌صورت لحظه‌ای پایش کند و برای هر پرسش تازه، خودش داده را کاوش کند."));
body.push(spacer(60));
