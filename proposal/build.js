...
body.push(new Table({
  ...
  rows: [
    ...
    ["[نام شرکت مجری — محل درج لوگو و مشخصات]", "مجری"],
    ["تیر ۱۴۰۵", "تاریخ"],
    ["نسخه ۱٫۰", "نسخه سند"],
...
body.push(spacer(80));
body.push(p("این پروژه در سه فاز پانزده‌روزه اجرا می‌شود و پس از استقرار، پشتیبانی و توسعهٔ نامحدودِ نیازهای شفاف‌سازی به‌صورت ماهیانه ارائه می‌گردد.", { bold: true, color: NAVY }));
body.push(spacer(40));
body.push(dataTable(
  ["عنوان", "شرح"],
  [
    ["مدت کل اجرا", "۴۵ روز (۳ فاز × ۱۵ روز)"],
    ["مبلغ کل قرارداد اجرا", "۲۶۵٬۰۰۰٬۰۰۰ تومان"],
    ["پشتیبانی و توسعهٔ نامحدود", "۲۵٬۰۰۰٬۰۰۰ تومان در ماه"],
...
body.push(dataTable(
  ...
  [
    ["پلتفرم عملیات", "ثبت هر عملیات سرویس/بازرسی لایف‌رفت", "شناسه عملیات، مشتری، تجهیز، اقلام تعویض‌شده، شعبه"],
    ["نرم‌افزار انبار", "کسر اقلام تعویض‌شده از موجودی", "کاردکس، ورود/خروج کالا، موجودی هر انبار"],
    ["نرم‌افزار حسابداری", "ثبت هزینه‌ها و درآمدهای هر عملیات", "اسناد مالی، درآمد، بهای تمام‌شده، سود/زیان"],
  ],
  [2400, 3260, 3700],
...
// Phase 1
body.push(h2("فاز اول — ساخت انبار داده (Data Warehouse) | ۱۵ روز"));
...
body.push(spacer(80));
// Phase 2
body.push(h2("فاز دوم — مایگریشن و متناظرسازی (Migration & Mapping) | ۱۵ روز"));
...
body.push(bullet("تعریف کلید مشترک و قواعد متناظرسازی بین عملیات، خروج انبار و اسناد مالی."));
body.push(bullet("پیاده‌سازی موتور مغایرت‌گیری سه‌جانبه (عملیات ⇄ انبار ⇄ حسابداری)."));
body.push(bullet("کشف و دسته‌بندی مغایرت‌ها: اقلام کسرنشده، اسناد مالی مفقود، درآمد بدون عملیات، خطای ثبت."));
...
body.push(dataTable(
  ...
  [
    ...
    ["مستمر", "پشتیبانی و توسعهٔ نامحدود نیازهای شفاف‌سازی (ماهیانه)"],
  ],
  [1800, 7560],
...
body.push(dataTable(
  ...
  [
    ["فاز ۱", "ساخت انبار داده", "۱۵ روز", "روز ۱ تا ۱۵"],
    ["فاز ۲", "مایگریشن و متناظرسازی", "۱۵ روز", "روز ۱۶ تا ۳۰"],
...
// ---------- 8. Support ----------
body.push(h1("۸. پشتیبانی و توسعهٔ مستمر"));
body.push(p("پس از اتمام سه فاز اجرا، هر درخواست جدید کارفرما در حوزهٔ شفاف‌سازی — از افزودن داشبورد و گزارش تازه تا تعریف قواعد مغایرت‌گیری جدید و پشتیبانی سامانه — به‌صورت نامحدود ارائه می‌شود."));
body.push(spacer(40));
body.push(dataTable(
  ["عنوان", "شرح"],
  [
    ["نوع خدمت", "پشتیبانی و توسعهٔ نامحدود نیازهای شفاف‌سازی"],
    ["مبلغ", "۲۵٬۰۰۰٬۰۰۰ تومان در هر ماه"],
    ["پوشش", "افزودن گزارش/داشبورد، قواعد مغایرت‌گیری جدید، رفع اشکال، به‌روزرسانی و پشتیبانی فنی"],
  ],
  [2900, 6460],
  { align: [AlignmentType.RIGHT, AlignmentType.RIGHT] }
));
...
body.push(dataTable(
  ["ردیف", "شرح", "مبلغ (تومان)"],
  [
    ["۱", "اجرای فاز اول — ساخت انبار داده", "—"],
    ["۲", "اجرای فاز دوم — مایگریشن و متناظرسازی", "—"],
    ["۳", "اجرای فاز سوم — پلتفرم BI و داشبوردها", "—"],
    ["", "جمع کل قرارداد اجرا (سه فاز)", "۲۶۵٬۰۰۰٬۰۰۰"],
    ["", "پشتیبانی و توسعهٔ نامحدود (ماهیانه)", "۲۵٬۰۰۰٬۰۰۰"],
  ],
  [1100, 6060, 2200],
  { align: [AlignmentType.CENTER, AlignmentType.RIGHT, AlignmentType.CENTER], boldCols: [] }
));
body.push(spacer(60));
body.push(p("مبلغ هر فاز در زمان عقد قرارداد بر اساس توافق طرفین از مبلغ کل تفکیک می‌شود؛ مبلغ کل قرارداد اجرا ۲۶۵ میلیون تومان است.", { size: 20, color: GREY }));
body.push(spacer(60));
...
body.push(bullet("هزینهٔ پشتیبانی ماهیانه از ماه پس از تحویل فاز سوم آغاز می‌شود."));
...
body.push(p("[نام و امضای شرکت مجری]", { color: GREY }));
